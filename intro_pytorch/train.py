...
def train(
    ...
    opt = optimizer(model.parameters(), 0.005)
    for i in range(epochs):
        ...
        trains.append(e_loss / len(train_loader))

        if val_loader is not None:
            eloss_ = 0

            for a in val_loader:
                data_val, labels_val = a
                data_val, labels_val = data_val, labels_val

                yhat_val = model(data_val)
                loss_ = criterion(yhat_val, labels_val)
                eloss_ = eloss_ + loss_.item()
                opt.zero_grad()
                loss_.backward()
                opt.step()

            vals.append(eloss_ / len(val_loader))

    return {"train":trains, "val":vals}
