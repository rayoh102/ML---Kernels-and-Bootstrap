...
def main():
    ...
    dataset_train = DataLoader(TensorDataset(torch.from_numpy(x).float(), torch.from_numpy(y)), batch_size=32, shuffle=True, generator=RNG,)
    dataset_val = DataLoader(TensorDataset(torch.from_numpy(x_val).float(), torch.from_numpy(y_val)), batch_size=32, shuffle=False,)
    dataset_test = DataLoader(TensorDataset(torch.from_numpy(x_test).float(), torch.from_numpy(y_test)), batch_size=32, shuffle=False,)
    ...
    plot_model_guesses(dataset_test, ce_configs[model]["model"], model + "predictions")

    plt.figure(figsize=(7, 5))
    temp = 0
    for key in ce_configs:
        plt.plot(range(100), ce_configs[key]["train"], "-", label=key + " training loss")
        plt.plot(range(100), ce_configs[key]["val"], "--", label=key + " testing loss")
        temp += 1
    plt.xlabel("Epochs")
    plt.ylabel("CE Loss")
    plt.legend()
    ...
    print("The accuracy is:", accuracy_score(ce_configs[model]["model"], dataset_test))
